...
- **machine catalog** — `refs/MachineRef.bmxml` holds one instance of every
  available machine; `refs/MACHINES.md` (generated by `src/catalog_machines.py`)
  tabulates each one's type, params/ranges, note column and editor columns.
  Splice any of them with `find_machine(read('refs/MachineRef.bmxml'), '<Library>')`.
